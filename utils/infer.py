...
class Args:
    ...
    def __post_init__(self) -> None:
        ...
        ts = datetime.now(pytz.timezone('UTC')).astimezone(
            pytz.timezone('US/Eastern'))

        self.ts = ts.strftime("%Y-%m-%d-%H-%M-%S")

        self.env = EnvironmentName(self.env_id)
        self.run_name = f"{self.algo.upper()}_{self.game}_{self.exp_id}_{self.ts}"
...
class InferLog:
    ...
    info_logs: List[Dict[str, Any]] = field(
    ...
    @property
    def run_frame_numbers(self) -> List[float]:
        return [episode.run_frame_number for episode in self.episode_logs]
...
def save_wandb_logs(infer_logs: List[InferLog],
    ...
    for infer_log in infer_logs:
        ...
        for episode in infer_log.episode_logs:
            ...
            run_frame_numbers.append(
                (episode.run_frame_number, episode.run_frame_number))
            episode_scores.append(
                (episode.run_frame_number, episode.episode_score))
            episode_lengths.append(
                (episode.run_frame_number, episode.episode_length))
            episode_times.append(
                (episode.run_frame_number, episode.episode_time))
            episode_lives.append(
                (episode.run_frame_number, episode.episode_lives))
            episode_frame_numbers.append(
                (episode.run_frame_number, episode.episode_frame_number))
...
def infer(run_key: str,
    ...
    stochastic = args.stochastic_on or (
        is_atari or is_minigrid) and not args.deterministic_on
    ...
    if args.progress:
        if tqdm is None:
            raise ImportError(
                "Please install tqdm and rich to use the progress bar")
